...
grep -Fq 'must currently use `&str`, `&[String]`, `&Struct`, or `&mut [f32; N]`' "$parser" || fail 'pure params are not constrained to verified borrowed string/list/fixed-array types'
grep -Fq 'cannot mix string/list borrows and `&mut [f32; N]`' "$parser" || fail 'mixed pure parameter ownership families are not rejected'
...
grep -Fq 'handler_kind != "pure fn"' crates/compiler/src/control_flow.rs || fail 'nested helper calls must remain unavailable inside pure functions until recursion is verified'

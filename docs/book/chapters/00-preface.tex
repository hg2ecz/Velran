...
Velran is a real server-side programming language focused on web applications, but it does not try to replace Rust as a systems language. For ordinary computation, keep the Rust mental model wherever the web-security contract does not require a framework-owned construct. The recurring \textbf{Rust $\rightarrow$ Velran} blocks show three things: what transfers unchanged, what the framework adds at a boundary, and what you should avoid relearning as a separate Velran concept.

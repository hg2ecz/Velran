...
Keep immutable release contents read-only where possible. For a transactional rolling-source site, use a dedicated application source directory owned by the deployment/upload workflow and readable by the Velran service; do not grant the application runtime arbitrary write access to its own source tree. In either model, the server should have write permission only where durable state, logs, or an explicitly managed native cache need it. Secret files must not live inside the application source/release directory or source control.
...
The repository contains a conservative baseline at \code{examples/systemd/velran.service}. The hardened shape below additionally shows the write path needed when a persistent native cache is configured:
...
The example also makes the persistent native-cache directory writable. If \code{native.cache_dir} points elsewhere, align the systemd filesystem sandbox with that exact directory instead of weakening \code{ProtectSystem=strict} globally. In rolling-source deployments, the upload workflow writes the application source tree; read access is sufficient for the Velran service.\par\medskip
...
\begin{enumerate}
  ...
  \item \textbf{HTTP edge:} verify domain, TLS, and Host handling; the trusted-proxy list contains only real proxies; certificate renewal ownership and expiry monitoring are explicit.
  ...
  \item \textbf{Resources/host:} request/runtime limits and process hard limits are consistent; cgroup authority is either systemd or a separately delegated Velran cgroup, not both; disk/inode headroom and host clock synchronization are healthy.
  ...
  \item \textbf{Deploy/rollback:} immutable deployments record the release artifact/checksum and rollback target; rolling-source deployments record the active source/generation identity and prove that a rejected candidate leaves the previous generation active. Schema changes have an explicit app-rollback versus full-restore decision.
...
\begin{itemize}[label={\texttt{[ ]}},leftmargin=2em]
  ...
  \item I can identify who owns TLS renewal, OS patching, time synchronization, disk/inode capacity, DB/Redis maintenance, and firewall/DNS changes, and I know which Velran checks must be repeated after those changes.

...
\path{reload.mode} & \code{development} & \code{development} or transactional \code{rolling}; strict production policy permits only \code{rolling}. \\
...
For a PHP-like rolling-production site where a web developer occasionally uploads source changes directly, add:
...
The developer does not need to manage release symlinks. Uploads, edits, renames, and deletions become candidate source-tree changes; only a stable, fully verified candidate is activated.


\section{Automatic source reload}

The global \code{[reload]} section defines \code{enabled}, \code{mode}, \code{poll_interval_ms}, \code{debounce_ms}, and the development-only \code{debug_compile_errors}. \code{mode} is either \code{development} or \code{rolling}; strict production policy permits only transactional \code{rolling} reload. A single shared supervisor fingerprints the Velran source tree with filesystem metadata plus SHA-256 content hashes. After debounce it builds and validates a candidate, fingerprints the live sources again, and discards the candidate if upload activity continued during compilation. Failed candidates leave the old generation active; successful candidates atomically replace only the affected domain and invalidate its public-cache generations. A consistent source deletion removes the corresponding module/handler/route from the next generation; a dangling reference rejects the candidate and preserves the previous generation. Requests already using the old \code{Arc<DomainRuntime>} finish on that generation. Use \code{--no-source-reload} for a process-wide maintenance override.

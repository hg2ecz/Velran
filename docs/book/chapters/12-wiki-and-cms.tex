...
For published content, prefer server-side Markdown rendering. The authoritative path is:
...
Keep the original Markdown in the database as the source of truth. Do not persist application-generated HTML merely to avoid rendering work. The server-side renderer rejects raw HTML and applies its link-target policy, so rendering remains inside the framework's HTML security boundary instead of moving trust into browser JavaScript.

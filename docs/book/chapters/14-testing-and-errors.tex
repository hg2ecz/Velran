...
Frontend expression and control-flow diagnostics preserve source location. File compilation should report the source path and line number, including errors inside nested \code{if}/\code{else}/\code{while} blocks. Treat a location-less generic parser message as a diagnostics-quality bug when the compiler has enough source information to do better.

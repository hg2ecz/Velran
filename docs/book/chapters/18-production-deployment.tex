...
\section{Two valid production update paths}

Velran supports two production-safe workflows. Choose one deliberately rather than mixing their filesystem assumptions.

\begin{description}
  \item[Immutable release deployment] Build a complete versioned release directory, verify it, switch the active path atomically, then restart or reload as required. Use this when operations owns explicit releases and migrations.
  \item[Transactional rolling source] Keep \code{reload.mode = "rolling"}; a web developer may upload, edit, rename, or delete watched \code{.vrn} files in place. The server fingerprints the settled source tree, builds and verifies a candidate away from live traffic, and atomically activates it only on success.
\end{description}

In both modes, partial source state must never become the active runtime. Database migrations remain explicit operator actions; rolling source activation does not run schema changes automatically.

\section{Immutable release path}
...
The built \code{velran-server} and \code{velran-cli} binaries, application source, and migrations should all be attributable to the same release identifier. In the immutable-release workflow, do not modify files one by one inside a live \code{/srv/velran/current} directory; prepare a new release directory and switch the symlink atomically. This restriction does not apply to a source tree intentionally operated in transactional \code{rolling} mode, where the candidate-generation boundary provides the atomicity.
...
Direct upload/edit/delete in rolling mode & stable fingerprint, candidate build, atomic generation swap & PHP-like workflow without release-directory manipulation \\
...
Treat activation as a two-step boundary: \emph{compile/validate first, activate second}. Failure leaves the known-good generation serving traffic. In transactional \code{rolling} mode, a web developer may upload, edit, rename, or delete watched \code{.vrn} files directly; the server treats the resulting source tree as a candidate rather than mutating the active runtime in place. Application developers can stop at this contract.

A source deletion follows the same transaction. If a module, handler, or route is removed together with all references to it, the next valid generation simply no longer serves that route. If any live source still references the missing module or symbol, candidate validation fails and the previous generation remains active. Deletion therefore either commits as part of one complete valid generation or has no effect on live traffic.

\subsection*{Under the hood: how native activation preserves the boundary}

This subsection is optional for application developers. The source-reload supervisor watches the logical entrypoint and relevant source tree using filesystem metadata plus content fingerprints. After the file set remains stable for the debounce period, compilation happens away from request handling. The candidate is checked against the same route, resource, cache, authentication and hosting constraints used at startup.
...
\begin{itemize}
  ...
  \item editing files directly inside an immutable active release directory while bypassing the transactional \code{rolling} supervisor;

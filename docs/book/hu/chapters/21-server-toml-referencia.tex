...
\path{reload.mode} & \code{development} & \code{development} vagy tranzakciós \code{rolling}; szigorú production policy alatt csak a \code{rolling} engedett. \\
...
PHP-szerű rolling-production oldalhoz, ahol a webfejlesztő időnként közvetlenül tölti fel a forrásmódosítást, add hozzá:
...
A fejlesztőnek nem kell release symlinket kezelnie. A feltöltés, szerkesztés, átnevezés és törlés candidate forrásfa-változás; csak stabil, teljesen verifikált candidate aktiválódhat.


\section{Automatikus source reload}

A globális \code{[reload]} section az \code{enabled}, \code{mode}, \code{poll_interval_ms}, \code{debounce_ms} és a fejlesztői \code{debug_compile_errors} alapértékeket adja. A \code{mode} értéke \code{development} vagy \code{rolling}; szigorú production policy alatt csak a tranzakciós \code{rolling} engedett. Egyetlen közös supervisor fájlrendszer-metaadat + SHA-256 tartalom-fingerprint alapján figyeli a Velran forrásfát. Debounce után candidate-et fordít és validál, majd aktiválás előtt újra fingerprintel; ha a feltöltés a build közben tovább változott, a candidate eldobódik. Sikertelen candidate esetén a régi generáció marad aktív; sikeres candidate csak az érintett domaint cseréli atomikusan és invalidálja annak public-cache generationjeit. Konzisztens forrástörlésnél a hozzá tartozó modul/handler/route eltűnik a következő generationből; dangling hivatkozás esetén a candidate elbukik és az előző generation marad aktív. A régi \code{Arc<DomainRuntime>} értéket már használó requestek azon fejeződnek be. Karbantartáskor a \code{--no-source-reload} kapcsoló process-szinten tiltja a figyelést.

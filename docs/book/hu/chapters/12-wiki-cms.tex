...
Publikált tartalomnál alapértelmezetten szerveroldali Markdown renderelést használj. A Markdown nem Markdown-specifikus engine feature: a \path{commonmark.vrn} forrásfájlt \code{mod commonmark;} deklarációval emeld be, és a Velranban megírt modult hívd. Az authoritative útvonal:
...
Nincs engine-szintű \code{@markdown(...)} direktíva. Az ismeretlen \code{@name(...)} alakú template call-direktívát a compiler elutasítja ahelyett, hogy literal szövegként renderelné. A renderelést normál Velran library-hívással, a \code{commonmark} modul \code{render} függvényével végezd.

Az adatbázisban az eredeti Markdown maradjon a source of truth. Ne tárold el az alkalmazás által generált HTML-t pusztán azért, hogy megspórold a renderelést. A Velranban megírt forrásmodul escape-eli a raw HTML-t és kezeli a link-policyt, miközben csak típusos \code{SafeHtml} értéket építhet. Így a Markdown szintaxis nincs az engine-be égetve, az általános HTML-security boundary viszont megmarad.
...
A canonical end-to-end példa az \path{examples/markdown-sql-cache/}. SQLite-, PostgreSQL- és MariaDB-sémaváltozatot is tartalmaz, és megmutatja, hogyan táplál egy id-alapú typed SQL olvasás \code{commonmark::render} hívást. Böngészőoldali Markdown renderelést csak szerkesztői preview-hoz használj, ha javítja az interakciót; a publikált reprezentációt továbbra is a szerver készítse, hogy a sanitization, cache-policy, CSP-feltételezések és JavaScript nélküli kliensek szempontjából a szerver maradjon authoritative.

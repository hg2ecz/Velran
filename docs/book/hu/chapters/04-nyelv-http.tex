...
\section{Verifikált pure számítás}

A verified pure függvény a Velran újrafelhasználható alkalmazás-compute rétege. Normál Rust-szerű \code{fn}, amely ambient web/server authority nélkül fut, és verifikált IR-en keresztül generált safe Rust kódra fordul. Parser, renderer, validációs helper, transzformáció, numerikus kernel és más olyan számítás való ide, amelynek nincs szüksége közvetlen adatbázis-, fájlrendszer-, hálózat-, process-, environment-, thread-, FFI- vagy \code{unsafe} hozzáférésre.

\subsection*{Paramétercsaládok és explicit borrow}

A scalar/borrowolt család immutable by-value \code{i64} és \code{bool}, valamint explicit immutable \code{\&str}, \code{\&[String]} és \code{\&Struct} paramétert támogat. A mutable numerikus hot-path család fix \code{\&mut [f32; N]} tömböt fogad. A két család szándékosan külön marad, amíg eltérő verifikált belső ABI-t használ.
...
A mutable tömb borrowja a hívási ponton is explicit marad:

\begin{lstlisting}[language=Velran]
kernel(&mut real)
\end{lstlisting}

A compiler nem értelmezi automatikusan a \code{kernel(real)} alakot mutable borrow-ként. Ez biztonsági és olvashatósági határ, nem fölösleges ceremónia.

\subsection*{Pure-to-pure hívások, rekurzió és budget}

Scalar/borrowolt pure függvény hívhat más scalar/borrowolt pure helpert normál típusos expression argumentumokkal, nem csak változónevekkel. A child munka ugyanabban az instruction- és allocation-elszámolásban marad; a helper hívás nem lehet budget nélküli kerülőút.
...
A scalar/borrowolt rekurzió támogatott, de runtime depth-bounded. A generált kód aktuális hard maximuma 64 nested pure hívás, az instruction- és allocation-budget mellett. A mutable numerikus hot-path családot érintő rekurzív ciklus compile-time fail-closed hiba.

A szabály tudatosan aszimmetrikus: normál scalar algoritmus használhat bounded rekurziót, ha az tisztább, miközben a speciális mutable-array ABI szigorúbb marad addig, amíg a compiler ugyanezt a biztonságot nem tudja bizonyítani azon a határon is.

\subsection*{Kontrollfolyam és return}

Verified pure kódban normál Rust-szerű \code{if}, \code{else if}, \code{else} és resource-budgetelt \code{while} használható. Minden feltétel pontosan egyszer értékelődik ki, és a compiler által létrehozott temporális megőrzi az eredeti statikus security metadata-t; untrusted boolból nem lesz csendben trusted bool.

A nested early \code{return} ágak a deklarált visszatérési típushoz képest típusellenőrzöttek. A jelenlegi lowering nem-unit függvénynél továbbra is megkövetel egy végső top-level returnt.
...
fn classify(value: i64) -> i64 {
    ...
    return 0; // jelenlegi final-return contract
}
\end{lstlisting}

\subsection*{Stringek és immutable builtin borrow}

A Rust-szerű string metódusok -- például \code{.trim()}, \code{.to_lowercase()} és \code{.to_string()} -- verifikált műveletek. A Stringet előállító munka allocation-budgetbe számít. String-orientált builtin argumentumhelyen explicit immutable borrow használható, ha a contract string-szerű értéket vár:
...
A compiler nem törli általánosan a \code{\&} jelet. Ahol a builtin contract nem fogad immutable borrow-t, ott compile error marad, és \code{\&mut} soha nem helyettesítheti az immutable string argumentumot.

\subsection*{SafeHtml és library-owned rendererek}

A \code{SafeHtml} típusos output boundary, nem ``trusted string'' konvenció. Normál \code{String} a \code{html \{ ... \}} interpolációban escape-elődik; csak \code{SafeHtml} kerül ki újabb escaping nélkül. A compiler/framework birtokolja az általános escapinget, SafeHtml konstruktorokat, tag- és URL-policyt, JSON-kezelést, typed SQL határokat és más webes biztonsági invariánsokat. A domain-specifikus renderelő logika application/library feladat.

Ezért a Markdown/CommonMark Velran forrásmodulban, például a \path{common/commonmark.vrn} fájlban él; nem engine feature. Szándékosan nincs \code{@markdown(...)} direktíva. Az ismeretlen \code{@name(...)} alakú template call-direktíva fail-fast compile errort ad ahelyett, hogy csendben literal szöveggé válna, így az elgépelések is azonnal látszanak.

\invariantblock{Első a biztonság, második a produktivitás}{Ne gyengíts authority-, type-, aliasing-, XSS-, SQL-, JSON- vagy resource-invariánst csak azért, hogy több szintaxist elfogadjunk. Ugyanakkor ne kényszerítsük az alkalmazáskódot mesterséges átírásra, ha egy normális konstrukció ezeknek az invariánsoknak a megtartásával implementálható.}
